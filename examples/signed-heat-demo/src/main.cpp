...
void solve() {
  ...
  if (MESH_MODE == MeshMode::Triangle) {
    if (SOLVER_MODE == SolverMode::ExtrinsicMesh) {
      ...
      determineSourceGeometryOnIntrinsicTriangulation(*intTri, curvesOnManifold, pointsOnManifold, curvesOnIntrinsic,
                                                      pointsOnIntrinsic);
      if (INTTRI_UPDATED) {
      ...
      ms_fp = t2 - t1;
      if (VERBOSE) {
        std::cerr << "min: " << PHI_CS.toVector().minCoeff() << "\tmax: " << PHI_CS.toVector().maxCoeff() << std::endl;
        std::cerr << "Solve time (s): " << ms_fp.count() / 1000. << std::endl;
      }
      ...
      if (!HEADLESS) {
        if (SHM_OPTIONS.levelSetConstraint != LevelSetConstraint::Multiple) {
          psMesh->addVertexSignedDistanceQuantity("GSD", PHI)->setEnabled(true);
          visualizeOnCommonSubdivision(*intTri, *manifoldGeom, csPositions, csGeom, PHI_CS, "GSD", true, true);
        } else {
          // If there's multiple level sets, it's arbitrary which one should be "zero".
          psMesh->addVertexScalarQuantity("GSD", PHI)->setIsolinesEnabled(true)->setEnabled(true);
          visualizeOnCommonSubdivision(*intTri, *manifoldGeom, csPositions, csGeom, PHI_CS, "GSD", true, false);
        }
      }
...
void callback() {
  ...
  ImGui::RadioButton("Constrain multiple levelsets", &CONSTRAINT_MODE, static_cast<int>(LevelSetConstraint::Multiple));
